...
export function webCaseStudyPublicRouter(publicContentRateLimit: RequestHandler) {
  ...
  router.get("/web-case-studies", publicContentRateLimit, async (_req: Request, res: Response) => {
    try {
  ...
  router.get("/web-case-study/:slug", publicContentRateLimit, async (req: Request, res: Response) => {
    try {
      const { slug } = req.params;
      const combined = await storage.getWebCaseStudyCombinedBySlug(slug);

      if (!combined) return res.status(404).json({ message: "Website case study not found" });
