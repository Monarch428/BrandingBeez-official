...
export function ThankYouPopup({
  ...
  // ✅ Scroll directly to popup (kept, but safe for all resolutions)
  useEffect(() => {
    if (isOpen && popupRef.current) {
      const t = setTimeout(() => {
        popupRef.current?.scrollIntoView({
  ...
  return (
    ...
    >
      ...
      {/* ✅ Responsive wrapper:
          - padding uses safe-area
          - allows scrolling on small screens
      */}
      <div className="relative z-50 w-full h-full overflow-y-auto overscroll-contain px-4 py-6 sm:px-6 sm:py-10">
        {/* Popup Container */}
        <div
          ref={popupRef}
          className={`relative mx-auto w-full max-w-[92vw] sm:max-w-md md:max-w-lg bg-white dark:bg-gray-800 rounded-2xl shadow-2xl transition-all duration-300 transform ${isClosing ? "scale-95 opacity-0" : "scale-100 opacity-100"
            }
          /* ✅ height behavior for 320px+ devices */
          max-h-[calc(100vh-3rem)] sm:max-h-[calc(100vh-5rem)]
          overflow-y-auto
          `}
          style={{
            // Better support for mobile safe-area notches
        ...
        >
          ...
          <div className="p-5 sm:p-7 md:p-8">
            ...
            {/* Next steps info */}
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 mb-5 sm:mb-6 border border-blue-200 dark:border-blue-800">
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-gray-700 dark:text-gray-300">
                  ...
                  <p>
                    Our team will review your{" "}
                    {formType === "strategy" ? "strategy request" : "submission"}{" "}
                    and contact you shortly to discuss how we can help.
                  </p>
                </div>
              </div>
            </div>
